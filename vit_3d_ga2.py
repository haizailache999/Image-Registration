...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=(10,10,10), in_c=1, num_classes=1000,
                 embed_dim=6*6*6, depth=1, num_heads=4, mlp_ratio=4.0, qkv_bias=True,
        ...
        self.embed_dim=embed_dim=6*6*6
        self.patch_embed = embed_layer(img_size=(10,10,10), patch_size=6, in_c=in_c, stride=2)
        num_patches = 3*3*3
        ...
        self.match_kv=nn.Linear(5,28*54)             #This should be calculated according your size
        self.block1 = Block2(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                  ...
                  norm_layer=norm_layer, act_layer=act_layer,dim1=344 * 16,dim2=5,layer_num=1)
    ...
    def forward(self, x,k,v):
        k=k.flatten(2)
        k=self.match_kv(k)
        k=k.view(x.shape[0],self.num_heads,28,54)
        v_initial=v.flatten(2)
        v=self.match_kv(v_initial)
        v=v.view(x.shape[0],self.num_heads,28,54)
        origin=x=x.float()
...
def own_model():
    model = VisionTransformer(img_size=(10,10,10), in_c=1, embed_dim=6*6*6)
    return model